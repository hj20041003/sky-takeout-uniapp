  ...
  (0, _vuex.mapState)(['orderData'])), {}, {
    ...
    handleSave: function handleSave() {var _this = this;
      if (this.timeout) {
        ...
        // 调用后端 /user/order/payment 接口
        uni.request({
          url: 'http://localhost:8080/user/order/payment',
          method: 'PUT',
          header: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            // token 只存在 vuex state 里（见 utils/request.js 的 storeInfo.token），不在 storage
            'authentication': _this.$store.state.token
          },
          data: { orderNumber: _this.orderDataInfo.orderNumber, payMethod: 1 },
          success: function(res) {
            if (res.data.code === 1) {
              // ============ 新增：模拟支付分支 ============
              if (res.data.data && res.data.data.mock === true) {
                wx.showModal({
                  title: '提示',
                  content: '支付成功（模拟）',
                  showCancel: false,
              ...
              // 后端返回业务错误（如"订单不存在"）或 HTTP 非 2xx（如 401 未登录）
              wx.showModal({
                title: '提示',
                content: (res.data && res.data.msg) || ('支付失败，请重新登录（' + res.statusCode + '）')
              })
            }
          },
          fail: function (err) {
