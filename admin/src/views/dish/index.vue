    ...
    <div class="container">
      ...
      <el-table v-if="tableData.length"
        ...
        <el-table-column label="库存"
                         width="90">
          <template slot-scope="scope">
            <span :style="{ color: scope.row.stock === 0 ? '#e95f3c' : '#333' }">{{ scope.row.stock }}</span>
          </template>
        </el-table-column>
        <el-table-column label="评分"
                         width="90">
          <template slot-scope="scope">
            <span>{{ scope.row.avgScore ? scope.row.avgScore + ' 分' : '-' }}</span>
          </template>
        </el-table-column>
...
export default class extends Vue {
  ...
  // 修改库存
  private stockHandle(row: any) {
    this.$prompt('请输入新的库存数量', '修改库存 - ' + row.name, {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      inputValue: String(row.stock || 0),
      inputValidator: (v: string) => {
        return /^\d+$/.test(String(v)) ? true : '请输入不小于0的整数'
      }
    }).then(({ value }: any) => {
      updateStock({ id: row.id, stock: Number(value) })
        .then(res => {
          if (res && res.data && res.data.code === 1) {
